...
declare -r XDG_DATA_HOME="${XDG_CONFIG_HOME}"
...
function main() {
  ...
  if [ -e "$LUNARVIM_CONFIG_DIR/init.lua" ]; then
    update_lvim
  else
...
function clone_lvim() {
  ...
  if ! git clone --branch "$LV_BRANCH" \
    --depth 1 "https://github.com/${LV_REMOTE}" "$LUNARVIM_CONFIG_DIR"; then
...
function link_local_lvim() {
  ...
  echo "   - $BASEDIR -> $LUNARVIM_CONFIG_DIR"
  ln -s -f "$BASEDIR" "$LUNARVIM_CONFIG_DIR"
...
function setup_lvim() {
  ...
  cp "$LUNARVIM_CONFIG_DIR/utils/installer/config.example.lua" "$LUNARVIM_CONFIG_DIR/config.lua"
}
